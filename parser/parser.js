...
var Parser = /** @class */ (function () {
    ...
    Parser.prototype.detectSectionStartAndEnd = function (keyword) {
        ...
        return { sectionStartIndex: sectionStartIndex, sectionEndIndex: sectionEndIndex };
    };
    Parser.prototype.splitSections = function (keyword) {
        var _a = this.detectSectionStartAndEnd(keyword), sectionStartIndex = _a.sectionStartIndex, sectionEndIndex = _a.sectionEndIndex;
    ...
    Parser.prototype.parseStimulusRow = function (row) {
        ...
        // 첫 토큰 2개 추출
        var stimulusType = tokens[0], identifier = tokens[1];
        ...
        switch (stimulusType) {
            case StimulusType.IMAGE:
                ...
                button = left[1];
                stimulus = __assign(__assign({}, stimulus), { filePath: filePath, button: !!button });
                break;
            case StimulusType.TEXT:
                var content = left[0], fontSize = left[1], fontColor = left[2];
                stimulus = __assign(__assign({}, stimulus), { content: content, fontSize: fontSize === "n" ? null : parseInt(fontSize), fontColor: fontColor === "n" ? null : fontColor });
                break;
            case StimulusType.TEXT_FILE:
                var filePath = left[0], fontSize = left[1], fontColor = left[2];
                stimulus = __assign(__assign({}, stimulus), { filePath: filePath, fontSize: fontSize === "n" ? null : parseInt(fontSize), fontColor: fontColor === "n" ? null : fontColor });
                break;
    ...
    Parser.prototype.parseSequenceRow = function (row) {
        ...
        switch (feedbackType) {
            case FeedbackType.ALWAYS:
                sequence = __assign(__assign({}, sequence), { feedback1: feedback1 == "n"
                        ? null
